...
from braindecode2.experiment import Experiment
...
def run(ex, data_folder, subject_id, n_chans,
    ...
    ival = [-500,3976]
    train_epo = segment_dat(train_cnt,
                         marker_def,
                         ival=ival)
    test_epo = segment_dat(test_cnt,
                         marker_def,
                         ival=ival)
    ...
    model = ShallowFBCSPNet(n_chans, n_classes,
                            final_dense_length=69).create_network()
    model.cuda()
    ...
    from braindecode2.stopcriteria import MaxEpochs, NoDecrease, Or
    stop_criterion = Or([MaxEpochs(1600),
                         NoDecrease('valid_misclass', 160)])

    from braindecode2.constraints import MaxNormDefaultConstraint
    from braindecode2.monitors import LossMonitor, MisclassMonitor, \
        RuntimeMonitor
