import braindevel.veganlasagne.monitors
from braindevel.datahandling.preprocessing import OnlineAxiswiseStandardize
...
import numpy as np
from braindevel.veganlasagne.update_modifiers import MaxNormConstraint
...
def test_experiment_fixed_split():
    ...
    monitors=[braindevel.veganlasagne.monitors.LossMonitor (),
                    braindevel.veganlasagne.monitors.MisclassMonitor(),
                    braindevel.veganlasagne.monitors.RuntimeMonitor()]
    stop_criterion= braindevel.veganlasagne.stopping.MaxEpochs(num_epochs=30)
...
def test_experiment_sample_windows():
    ...
    loss_var_func= lasagne.objectives.categorical_crossentropy
    monitors=[braindevel.veganlasagne.monitors.LossMonitor (),
                    braindevel.veganlasagne.monitors.WindowMisclassMonitor(),
                    braindevel.veganlasagne.monitors.RuntimeMonitor()]
    stop_criterion= braindevel.veganlasagne.stopping.MaxEpochs(num_epochs=5)
